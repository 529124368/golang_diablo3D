...
//Add zimuge
func NewTexture2DFromImagePlius(datas []byte) (*Texture2D, error) {

	// Decodes image file into RGBA8
	rgba, err := DecodeImagePlus(datas)
	if err != nil {
		return nil, err
	}
...
//Add zimuge
func DecodeImagePlus(datas []byte) (*image.RGBA, error) {

	// Decodes image
	img, _, err := image.Decode(bytes.NewReader(datas))
	if err != nil {
		return nil, err
	}
